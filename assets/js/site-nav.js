/*
 * 站点浮动导航：回到主页 + 收藏本站
 * 全站共用，注入到每个 HTML 的 </body> 前。
 * - 非主页显示「← 回到主页」（链接 index.html）
 * - 所有页面显示「★ 收藏本站」（浏览器禁止 JS 直接加书签，故点击弹出 Ctrl/Cmd+D 提示，收藏【当前页】）
 * 自包含：样式由本脚本注入，不依赖任何外部 CSS，避免样式缺失导致导航失效。
 */
(function () {
  ...
  var css = [
    ...
    '.sn-btn{display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#0a66c2,#0a8fae);color:#fff;border:0;border-radius:999px;',
    '  padding:11px 16px;font-size:14px;font-weight:600;cursor:pointer;box-shadow:0 4px 14px rgba(10,102,194,.35);',
    '  text-decoration:none;font-family:inherit;line-height:1;transition:transform .15s ease, box-shadow .15s ease}',
    '.sn-btn:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(10,102,194,.45)}',
    '.sn-btn.star{background:linear-gradient(135deg,#0e7c66,#0a8f7a)}',
    '.sn-btn.star:hover{background:linear-gradient(135deg,#0b6654,#0a7a68)}',
    ...
    '  opacity:0;transition:opacity .25s ease, transform .25s ease;pointer-events:none;max-width:92vw;text-align:center;line-height:1.7}',
  ...
  star.addEventListener('click', function () {
    ...
    showToast('按 ' + hotkey + ' 将本页 <b>' + url + '</b> 加入收藏夹');
  });
